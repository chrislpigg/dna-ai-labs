...
test("PostgreSQL reads are tenant-scoped and serialize portfolio evidence, reviews, and decisions", async () => {
  const database = new QueryMock([
    ...
    { rows: [{ project_id: "project-1", metric_key: "primary", source_type: "analytics_dashboard", source_ref: "dashboards/adoption-readiness", hypothesis_label: "Expected review time reduction", verified_value: "42 teams", verified_at: "2026-06-20T00:18:00.000Z", stale_at: "2026-08-01T00:00:00.000Z", refresh_status: "verified", last_error_code: null, updated_at: "2026-06-20T00:18:00.000Z", updated_by: "user-1" }] },
    { rows: [{ project_id: "project-1", event_key: "decision_meeting:decision-1", event_type: "decision_meeting", decision_id: "decision-1", provider: "calendar", external_ref: "event-1", external_url: "https://calendar.example/events/event-1", scheduled_for: "2026-07-10T16:00:00.000Z", last_verified_at: "2026-06-20T00:20:00.000Z", created_by: "user-1", created_at: "2026-06-20T00:20:00.000Z", updated_by: "user-1", updated_at: "2026-06-20T00:20:00.000Z" }] },
    { rows: [{ project_id: "project-1", due_on: "2026-08-01", status: "pending", reminder_notification_id: "notification-1", created_at: "2026-06-20T00:25:00.000Z", created_by: "user-3", completed_at: null, completed_by: null }] },
  ...
  assert.equal(project.metricPlan.refreshStatus, "verified");
  assert.equal(project.metricPlan.verifiedValue, "42 teams");
  assert.equal(project.calendarEvents[0].eventType, "decision_meeting");
  assert.equal(project.calendarEvents[0].scheduledFor, "2026-07-10T16:00:00.000Z");
  ...
  assert.equal(database.calls.length, 12);
...
test("PostgreSQL cycle reads serialize capacity and steering metadata", async () => {
  const database = new QueryMock([
    { rows: [{ id: "cycle-1", name: "Cycle 02", theme: "Operational readiness", starts_on: "2026-10-01", ends_on: "2026-12-31", capacity_units: 4, steering_group_ids: ["lab-lead"], status: "planned" }] },
    { rows: [{ id: "cycle-1", name: "Cycle 02", theme: "Operational readiness", starts_on: "2026-10-01", ends_on: "2026-12-31", capacity_units: 4, steering_group_ids: ["lab-lead"], status: "planned" }] }
  ]);
  const adapter = new PostgresReadAdapter({ queryable: database, organizationId: "org-a" });

  assert.equal((await adapter.listCycles())[0].capacityUnits, 4);
  assert.deepEqual((await adapter.getCycle("cycle-1")).steeringGroupIds, ["lab-lead"]);
  assert.deepEqual(database.calls[0].params, ["org-a"]);
  assert.deepEqual(database.calls[1].params, ["org-a", "cycle-1"]);
...
test("PostgreSQL role assignment reads are tenant scoped", async () => {
  const database = new QueryMock([
    { rows: [{ user_id: "user-1", assigned_role: "submitter", active: true, assigned_by: "admin", assigned_at: "2026-06-20T00:00:00.000Z" }] },
    { rows: [{ user_id: "user-1", assigned_role: "submitter", active: true, assigned_by: "admin", assigned_at: "2026-06-20T00:00:00.000Z" }] }
  ]);
  const adapter = new PostgresReadAdapter({ queryable: database, organizationId: "org-a" });

  assert.equal((await adapter.listRoleAssignments())[0].role, "submitter");
  assert.equal((await adapter.getRoleAssignment("user-1")).assignedBy, "admin");
  assert.deepEqual(database.calls[0].params, ["org-a"]);
  assert.deepEqual(database.calls[1].params, ["org-a", "user-1"]);
