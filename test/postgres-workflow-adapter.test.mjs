...
test("PostgreSQL workflow writes project, evidence, review, decision, approval, and handoff state with its audit event in one transaction", async () => {
  ...
  for (const table of ["cycles", "feature_flags", "intake_drafts", "intake_draft_collaborators", "projects", "intake_revisions", "project_triage_comments", "evidence_entries", "project_reviews", "decisions", "approvals", "handoffs", "audit_events"]) assert.equal(sql.some(statement => statement.includes(`INSERT INTO ${table}`)), true);
